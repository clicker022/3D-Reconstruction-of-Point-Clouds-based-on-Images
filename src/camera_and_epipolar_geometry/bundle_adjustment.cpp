...
void convert_sift_discriptors(sfm::Sift::Descriptors const&sift_descrs,
                              util::AlignedMemory<math::Vec128f, 16> *aligned_descr)
{
    ...
    {
        sfm::Sift::Descriptor const& descr = sift_descrs[i];
...
{
    std::string exif_str;
    mve::image::load_jpg_file(img_name.c_str(), &exif_str);
        ...
        , sfm::FeatureSet&feat2)
{
    ...
    sfm::Matching::twoway_match(matching_opts, aligned_descrs1.data()->begin()
    ...
    int n_consitent_matches = sfm::Matching::count_consistent_matches(matching_result);
    ...
    // 保持计算的稳定性
    feat1.normalize_feature_positions(0.5f, 0.5f);
    feat2.normalize_feature_positions(0.5f, 0.5f);
...
main (int argc, char *argv[])
{
    ...
    std::ofstream out("../../camera_and_epipolar_geometry/test_ba.txt");
    assert(out.is_open());
